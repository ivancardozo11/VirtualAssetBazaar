...
const sendErc20Tokens = async (TOTAL_TOKENS_FOR_SALE, BUYER_WALLET_ADDRESS) => {
    const erc20Contract = new web3.eth.Contract(mockErc20ABI, MOCK_ERC20_ADDRESS);
    const nonce = await web3.eth.getTransactionCount(SELLER_ADDRESS, 'pending');
    const data = erc20Contract.methods.transfer(BUYER_WALLET_ADDRESS, TOTAL_TOKENS_FOR_SALE).encodeABI();
